...
from llava_quant.llava_wa.persistence import _jsonable_config
...
def run_grid_search_fp4_reconstruction(args: argparse.Namespace) -> dict[str, Any]:
    ...
    # for name, values in train_data.items():
    #     print(f"train_data[{name}]: {values.shape}")
    #     break

    # print(f"len(validation_data): {len(validation_data)}")
    # for name, values in validation_data.items():
    #     print(f"validation_data[{name}]: {values.shape}")
    #     break
...
# def run_grid_search_fp4_kl(args: argparse.Namespace) -> dict[str, Any]:
#     """Collect activations, evaluate all FP4 trials, and persist the best model."""
#     _validate_args(args)
#     trials = _trial_configs(args)
#     try:
...
#         quantize_language=args.quantize_language,
#     )
#     train_data, _ = split_calibration_data(
#         all_data, args.validation_fraction, args.seed
#     )
#     del all_data
#     _release_cuda_memory()
...
#     # 
#     eval_pairs = load_data_for_eval_logits(
#         calibration_sample_num=args.calibration_samples,
#         eval_sample_num=32,
#         prompt_text=args.prompt,
#         image_column=args.image_column,
...
#         score, detail_list = reconstruction_score_logits_kl(
#             model,
#             eval_pairs,
#             max_new_tokens=args.inference_max_new_tokens,
#         )

#         print(f"Trial {trial_index} validation reconstruction score: {score:.8g}")
...
#             "search_parameters": {key: getattr(cfg, key) for key in SEARCH_GRID_KEYS},
#             "loss_weights": {key: getattr(cfg, key) for key in LOSS_GRID_KEYS},
#             "validation_reconstruction": score,
#             # "layer_validation_mse": layer_scores,
#             "final_training_losses": {
#                 name: {
#                     kind: history[-1] if history else None
...
#                 "format_version": 1,
#                 "base_checkpoint": CHECKPOINT,
#                 "selection_metric": "validation_reconstruction_kl",
#                 "best_trial": best_trial,
#                 "best_score": best_score,
#                 "assistant_outputs_file": str(Path("..") / inference_path.name),
#                 "surequant": _jsonable_config(cfg),
...
def main() -> None:
    ...
    start = time.time()
    summary = run_grid_search_fp4_reconstruction(args)
