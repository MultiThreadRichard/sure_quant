...
def calibrate_all_quantizers(
    ...
    for index, (name, module) in enumerate(modules, start=1):
        ...
        if name in calibration_data and module.activation_quantizer is not None:
...
def weight_reconstruction_score(model: nn.Module) -> tuple[float, dict[str, float]]:
    """Calculate weight reconstruction MSE for all wrapped layers.

    Used as the selection metric in weight-only mode, where activations are
    not quantized and there is no activation reconstruction to measure.
    """
    ...
    for name, module in model.named_modules():
        if not isinstance(module, SureQuantLinear) or module.weight_quantizer is None:
            continue
        device = module.linear.weight.device
        quantizer = module.weight_quantizer.to(device).eval()
        weight_t = module.linear.weight.detach().T.contiguous()
        output = quantizer(weight_t)
        difference = output["x_blk"] - output["x_hat_blk"]
        layer_scores[name] = float(difference.float().square().mean())
    if not layer_scores:
        raise RuntimeError("No weight-quantized layer found for scoring")
    ...
    *,
    ...
    for name, module in model.named_modules():
        if not isinstance(module, SureQuantLinear) or name not in validation_data:
            continue
        if module.activation_quantizer is None:
            continue
