...
def plt_llava_vision_weight(model, output_path, stat_axis=0):
    ...
    for idx, layer in enumerate(model.vision_tower.vision_model.encoder.layers):
        ...
        for attr_name in attr_targets:
            ...
            # SureQuantLinear
            # print(f"layer {idx} {attr_name}: {target_module.linear.weight.shape}")
            if stat_axis == 0:
                weight_matrix = target_module.linear.weight.T.detach().cpu().float().numpy()  # 转float32避免numpy计算精度问题
            else:
                weight_matrix = target_module.linear.weight.detach().cpu().float().numpy()
