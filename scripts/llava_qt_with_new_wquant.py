...
from model.sure_quantizer import SureQuantizer, WeightCalibFreeQuantizer
...
无校准
权重量化修改：
SureQuantizer -> WeightCalibFreeQuantizer
...
def quantize_linear_layer(
    ...
    if quantize_weight and linear.out_features % block_size == 0:
        weight_quantizer = WeightCalibFreeQuantizer(
...
def apply_weight_quantization(quantized_model: LlavaForConditionalGeneration):
    """对量化模型中的所有 SureQuantLinear 层应用权重量化

    Args:
        quantized_model: 已量化的模型（包含 SureQuantLinear 层）
    """
    # layer_count = 0
    # total_layers = sum(1 for _, m in quantized_model.named_modules() if isinstance(m, SureQuantLinear))
    ...
    for name, module in quantized_model.named_modules():
        if isinstance(module, SureQuantLinear):
            # layer_count += 1
            if module.weight_quantizer is not None:
                print(f">>>>>>>> Applying weight quantization to {name}")
                module.quantize_weight()
            else:
                print(f">>>>>>>>  {name} has no weight quantizer, skipping")

    print(">>>>>>>> Weight quantization completed")
...
def example_calib():
    ...
    # 应用权重量化
    apply_weight_quantization(quantized_model)
    ...
    # 保存量化后的模型
    save_path = "/home/ccwan/stu_Jiangtp/sure_quant/model_saved/llava_7b_4bit_blk128_w_calibfree"
    os.makedirs(save_path, exist_ok=True)
    save_quantized_model(quantized_model, save_path, cfg)


def infer_saved_model():
    save_path = "/home/ccwan/stu_Jiangtp/sure_quant/model_saved/llava_7b_4bit_blk128_w_calibfree"
    checkpoint = "/home/ccwan/stu_Jiangtp/model_repo/llava-7b-hf"
