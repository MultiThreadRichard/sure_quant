...
class KVCacheCompressor:
    ...
    def memory_stats(self, seq_len: int, num_layers: int, num_heads: int) -> dict:
        ...
        original_bytes = 2 * n_vectors * self.head_dim * 2  # first ×2 for K+V, second ×2 for fp16

        # K 需要两个范数（vector_norms + residual_norms），共64 bits per vector
        k_bits_total = n_vectors * (self.head_dim * self.k_bits + 64)
        # V 需要一个范数（vector_norms），共32 bits per vector
