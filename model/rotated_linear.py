...
class RotatedQuantLinear(nn.Module):
    """Linear layer preceded by rotation quantisation of its input.

    Args:
        linear: Original ``nn.Linear`` layer.
        sure_quantizer: A pre‑trained ``RotationQuantizer`` instance.
    """

    def __init__(self, linear: nn.Linear, sure_quantizer: RotationQuantizer):
        super().__init__()
        if linear.in_features != sure_quantizer.dim:
            raise ValueError(
                f"Linear in_features={linear.in_features} must match "
                f"RotationQuantizer dim={sure_quantizer.dim}"
            )
        self.linear = linear
